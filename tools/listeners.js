document.getElementById('convertAddressesBtn').addEventListener('click', async () => {
    ...
    for (const addr of addresses) {
        if (!hasNewAddress) outputElement.innerHTML += '<hr>'; 
        hasNewAddress = true;

        const lowercaseAddress = addr.toLowerCase();
        outputElement.innerHTML += `
            <div class="result"><strong>Checksum Address:</strong> <span class="address">${addr}</span>
            <button class="copy-btn" onclick="copyToClipboard(this, '${addr}')">copy</button>
            <span class="copied">Copied!</span></div>
            <div class="result"><strong>Lowercase Address:</strong> <span class="address">${lowercaseAddress}</span>
